...
class Problem3i():
    def __init__(self):
        ...
        self.PutAbsorbingBarrier = False #Puts a Barrier That Absorbs the Waves ---> Diffraction
    ...
    def updatePointSource(self):
        self.u[0, self.IndexWhenXisMinus2_5] = 5 * np.sin(np.pi * self.t * 2) + .25
        self.u[0, self.IndexWhenXisPlus2_5]  = 5 * np.sin(np.pi * self.t * 2) + .25
        self.s[0, self.IndexWhenXisPlus2_5]  = 5 * np.pi * np.cos(np.pi * self.t * 2)
        self.s[0, self.IndexWhenXisMinus2_5] = 5 * np.pi * np.cos(np.pi * self.t * 2)
